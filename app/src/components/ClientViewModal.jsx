...
// /** ---------- helpers (no hardcoded display data) ---------- */
// const fmtMD = (d) => {
//   if (!d) return "—";
//   const dt = new Date(d);
//   if (isNaN(dt)) return "—";
//   return dt.toLocaleDateString(undefined, { month: "short", day: "numeric" }); // "Sep 15"
// };
// const fmtDate = (d) => {
//   if (!d) return "—";
//   const dt = new Date(d);
...
// };
// const phaseClass = (p) => (p >= 100 ? "completed" : p > 0 ? "in-progress" : "pending");
...
//     <div className="mb-4">
//       <div
//         className="flex items-center justify-between px-4 py-3 rounded-md border border-gray-200 bg-white cursor-pointer"
//         onClick={() => setOpen((o) => !o)}
//       >
...
//         </div>
//       </div>
//       {open && <div className="bg-white rounded-md border border-t-0 border-gray-200 p-4">{children}</div>}
//     </div>
//   );
// }
...
//   // fetch all data in parallel (fixed number of hooks; no conditional hooks => no Hook order bugs)
//   useEffect(() => {
//     let cancelled = false;
...
//         const chronoQ = supabase.from("chronology_entries").select("*").eq("project_id", project.id).order("entry_date", { ascending: false });
//         const docQ = supabase.from("project_documents").select("*").eq("project_id", project.id).order("uploaded_at", { ascending: false });
//         const shipQ = supabase.from("shipments").select("*").eq("project_id", project.id).order("planned_date", { ascending: true });
//         const qaQ = supabase.from("quality_checks").select("*").eq("project_id", project.id).order("check_date", { ascending: false });
//         const riskQ = supabase.from("project_risks").select("*").eq("project_id", project.id).order("severity", { ascending: false });
//         const stakeQ = supabase.from("project_stakeholders").select("*").eq("project_id", project.id).order("role", { ascending: true });
//         const mileQ = supabase.from("project_milestones").select("*").eq("project_id", project.id).order("target_date", { ascending: true });
...
//   if (loading) {
//     return (
//       <div className="client-view active">
//         <div className="client-view-container flex items-center justify-center h-64">
//           <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-blue-500" />
//           <p className="ml-4 text-lg text-gray-700">Loading Project Details...</p>
//         </div>
//       </div>
//     );
//   }
...
//     return (
//       <div className="client-view active">
//         <div className="client-view-container p-8 text-center">
//           <h2 className="text-xl font-semibold text-red-600 mb-4">Error Loading Client View</h2>
...
//           <p className="text-gray-700 mb-6">The selected project could not be loaded. It might have been deleted or does not exist.</p>
//           <button className="client-close-btn" onClick={onClose}>✕ Close</button>
//         </div>
//       </div>
//     );
...
//   /** ---------- computed ---------- */
//   const equipmentCount = Array.isArray(proj?.equipment) ? proj.equipment.length : (proj?.equipment_count || 0);
...
//             Project Status Update • PO: {proj?.po_number}
//           </div>
//         </div>
...
//               <div className="client-status-number">{equipmentCount}</div>
//               <div className="client-status-label">Total Equipment</div>
//             </div>
//             <div className="client-status-card">
//               <div className="client-status-number">{fmtMD(proj?.expected_delivery_date)}</div>
//               <div className="client-status-label">Expected Delivery</div>
//             </div>
//             <div className="client-status-card">
//               <div className="client-status-number">{overall}%</div>
//               <div className="client-status-label">Overall Progress</div>
//             </div>
//           </div>

//           {/* Phases */}
//           <div className="client-progress-section">
//             <h3 className="client-progress-title">Project Phases</h3>
//             {phases.length ? (
//               phases.map((p) => (
//                 <div key={p.name} className={`client-phase ${phaseClass(p.progress)}`}>
//                   <div className="client-phase-name">{p.name}</div>
//                   <div className="client-phase-status">{phaseText(p.progress)}</div>
//                 </div>
//               ))
//             ) : (
...
//             <h3 className="client-progress-title">Recent Updates</h3>
//             {chronology.length ? (
//               chronology.map((item, idx) => (
//                 <div key={idx} className="client-timeline-item">
//                   <div className="client-timeline-date">{fmtDate(item.entry_date)}</div>
//                   <div className="client-timeline-content">
//                     <div className="client-timeline-title">{item.title}</div>
//                     <div className="client-timeline-desc">{item.description}</div>
//                   </div>
//                 </div>
//               ))
//             ) : (
...
//           {/* -------- Optional / Extended blocks (auto-hide if empty) -------- */}

//           {Array.isArray(proj?.equipment) && proj.equipment.length > 0 && (
//             <Collapse
//               title="Equipment Breakdown"
//               right={<span className="text-xs text-gray-500">{proj.equipment.length} items</span>}
//             >
//               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
//                 {proj.equipment.map((eq) => (
//                   <div key={eq.id} className="equipment-item">
//                     <div className="flex items-center justify-between mb-1">
//                       <div className="font-medium text-gray-800">{eq.type || "Equipment"}</div>
//                       {pct(eq.unit_progress) > 0 && (
//                         <span className="text-xs text-gray-600">{pct(eq.unit_progress)}%</span>
//                       )}
//                     </div>
//                     <div className="text-sm text-gray-600">
//                       <span className="mr-3">Tag: <b>{eq.tag_number || "—"}</b></span>
//                       <span>Serial: <b>{eq.manufacturing_serial || "—"}</b></span>
//                     </div>
//                   </div>
//                 ))}
//               </div>
...
//                     <div>
//                       <div className="font-medium text-gray-800">{d.title || d.file_name}</div>
//                       <div className="text-gray-500 text-xs">Uploaded: {fmtDate(d.uploaded_at)} • Type: {d.doc_type || "—"}</div>
//                     </div>
//                     {/* assuming you store a public URL in d.url; else sign URL before linking */}
//                     {d.url && (
//                       <a className="btn btn-outline btn-small" href={d.url} target="_blank" rel="noreferrer">
//                         View
//                       </a>
//                     )}
//                   </li>
...
//                         <td className="py-2">{s.title || s.item_code || "—"}</td>
//                         <td className="py-2">{fmtDate(s.planned_date)}</td>
//                         <td className="py-2">{fmtDate(s.actual_date)}</td>
//                         <td className="py-2">
...
//                       <div className="font-medium text-gray-800">{q.title || q.check_type}</div>
//                       <div className="text-xs text-gray-500">
//                         {fmtDate(q.check_date)} • Result: <b>{q.result || "—"}</b> {q.tpi_agency ? `• TPI: ${q.tpi_agency}` : ""}
//                       </div>
//                       {q.remarks && <div className="text-sm text-gray-600 mt-1">{q.remarks}</div>}
//                     </div>
//                   </div>
//                 ))}
//               </div>
...
//                       </div>
//                     </div>
//                     <div className="text-xs">{phaseText(pct(m.progress))}</div>
//                   </li>
//                 ))}
//               </ul>
//             </Collapse>
...
//               <ul className="space-y-2">
//                 {risks.map((r) => (
//                   <li key={r.id} className="equipment-item">
//                     <div className="flex items-center justify-between">
//                       <div className="font-medium text-gray-800">{r.title}</div>
//                       <span className="text-xs text-red-600">Severity: {r.severity}</span>
//                     </div>
//                     {r.mitigation && <div className="text-sm text-gray-600 mt-1">Mitigation: {r.mitigation}</div>}
...
//               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
//                 {stakeholders.map((p) => (
//                   <div key={p.id} className="equipment-item">
//                     <div className="font-medium text-gray-800">{p.name}</div>
//                     <div className="text-xs text-gray-500">{p.role}</div>
//                     <div className="text-sm text-gray-600 mt-1">{p.email || "—"} {p.phone ? `• ${p.phone}` : ""}</div>
//                   </div>
//                 ))}
//               </div>
//             </Collapse>
